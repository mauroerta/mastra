...
Mastra supports A2A Protocol versions 0.3 and 1.0 of the [Agent-to-Agent (A2A) protocol](https://a2a-protocol.org/latest/) for cross-platform multi-agent systems. Use A2A to expose Mastra agents as remote agents or consume remote A2A agents as Mastra subagents. You can also call A2A endpoints with the JavaScript client SDK.
...
An agent card includes fields like the agent name, description, endpoint URL, provider, capabilities, security metadata, and skills. Requests without `A2A-Version` receive a 0.3-compatible card that still advertises both interfaces when dual exposure is enabled:
...
A request with `A2A-Version: 1.0` receives a v1 card that uses `supportedInterfaces` as the source of truth for execution URLs and protocol versions. Card responses include `Vary: A2A-Version`.
...
- Missing, empty, or `0.3`: Uses the v0.3 slash-style JSON-RPC methods such as `message/send`.
- `1.0`: Uses the v1.0 PascalCase methods such as `SendMessage`, `SendStreamingMessage`, `GetTask`, `ListTasks`, and `SubscribeToTask`.
- Any other value: Returns a `VersionNotSupported` protocol error.

By default, Mastra exposes both versions in preference order `['1.0', '0.3']`. Configure exposure on the server, and optionally override it per agent:
  ...
  server: {
    a2a: {
      ...
      agents: {
        ...
        'v1-only-agent': { protocolVersions: ['1.0'] },
      },
    },
  },
})
...
A per-agent list replaces the server list. Mastra only advertises versions it can serve for that agent. Requests for an unadvertised version return `VersionNotSupported`.

Existing `MastraClient.getA2A(agentId)` calls continue to use v0.3. Prefer `getA2A(agentId, { protocolVersion: '1.0' })` for v1.0, or keep `getA2AV1()` as a compatibility alias. Use `await getA2A(agentId, { protocolVersion: 'auto' })` to select from the agent card. The v1 client sends `A2A-Version: 1.0` automatically and supports `ListTasks`.

`A2AAgent` defaults to `protocolVersion: 'auto'` and selects a supported JSON-RPC interface from the remote card. Pin `0.3` or `1.0` when the remote agent must use one version.

With `'auto'`, a remote that advertises both versions (for example another Mastra server) resolves to the first advertised interface, which is `1.0`. Pin `protocolVersion: '0.3'` to keep the 0.3 wire version against such a remote. A legacy 0.3 card that exposes only a top-level `url` (no `supportedInterfaces`, and `protocolVersion` absent or `0.3`) still resolves to 0.3, so bare-`url` 0.3 remotes work without extra configuration.

Passing an `A2A-Version` header that conflicts with the resolved or pinned version, including the `'auto'` default, throws at construction. Pin `protocolVersion` instead of setting the header.
...
- Selects a JSON-RPC interface from the card with `protocolVersion: 'auto'`, or uses a pinned `0.3` or `1.0` value.
- Reads the execution URL and capabilities for the selected interface.
- Calls the version-correct send, stream, get-task, and subscribe methods.
...
Use `getA2A(agentId, { protocolVersion: '1.0' })` to opt into the A2A v1.0 wire protocol. `getA2AV1()` remains available as a compatibility alias. The protocol package provides codecs for creating v1 request values from JSON-shaped input:
...
The v1.0 client supports `getAgentCard()`, `sendMessage()`, `sendMessageStream()`, `getTask()`, `listTasks()`, `cancelTask()`, `resubscribeTask()`, and the v1 push-notification configuration methods.
...
You can also pass `credentials`, `fetch`, and `abortSignal` when the runtime needs custom fetch behavior or request cancellation. Use `protocolVersion: 'auto'` when the remote agent may advertise either version.
...
Mastra Server sends the current task snapshot to registered callbacks when the task reaches `completed`, `failed`, `canceled`, `rejected`, `input-required`, or `auth-required`. Delivery uses the protocol version that registered the callback: 0.3 callbacks receive a bare 0.3 task with `application/json`, and v1 callbacks receive a v1 task wrapper with `application/a2a+json`. Push notification delivery is best-effort. Protect callback URLs, validate notification tokens, and avoid exposing internal network targets as push notification destinations.
...
Mastra canonicalizes the card for signing per version so signatures verify against any A2A peer: v1.0 cards use the `@a2a-js/sdk` canonicalization (which strips empty `securitySchemes`, `securityRequirements`, and `extensions` before hashing), and v0.3 cards use RFC 8785 JSON Canonicalization Scheme (JCS). To verify a signed v1.0 card against an `@a2a-js/sdk` peer, include `kid` and `typ` in `protectedHeader` alongside `alg`.
