...
The ⁠`MastraServer` is initialized with the existing Express `app` and the root ⁠`mastra` instance. `mastraJsonBodyParser()` parses JSON request bodies, including A2A Protocol v1.0's `application/a2a+json`. Calling `⁠init()` registers the Mastra middleware and exposes all available Mastra endpoints.
