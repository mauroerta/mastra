...
Koa doesn't parse request bodies on its own, and common parsers such as `koa-bodyparser` accept only `application/json`. `mastraJsonBodyParser()` is a dependency-free parser that also handles A2A Protocol v1.0 requests, which arrive as `application/a2a+json`. Without it, those requests fail with `Method not found: undefined`. Add the parser before creating the `MastraServer`. To keep `koa-bodyparser` instead, extend its accepted types to include `application/*+json`.
